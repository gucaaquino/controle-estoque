...
public class GerenciarLocaisVisao extends JFrame{
	...
	private ArrayList<LocalizacaoVO> itensTabela;
	...
	//construtor
	public GerenciarLocaisVisao(LumanferControle controle, LumanferMenu menu) {
		super("Gerenciar Locais");
	...
	private void inicializaComponentes() {
		...
		buttonAdicionar.setToolTipText("Adicionar novo local");
		buttonAlterar.setToolTipText("Alterar local");
		buttonDeletar.setToolTipText("Deletar local");
		buttonVoltar.setToolTipText("Voltar a tela inicial");
		buttonVisualizar.setToolTipText("Visualizar local");
		...
		itensTabela = new ArrayList<LocalizacaoVO>();
	...
	public void atualizaTabela() {
		...
		if(radioCodigo.isSelected()) 
			itensTabela = controle.getLocalizacoes(pesquisa, "");
		else
			itensTabela = controle.getLocalizacoes("", pesquisa);

		for(LocalizacaoVO l : itensTabela) {
			//as linhas da tabela devem ser do tipo Object
			tableModel.addRow(new Object[] {
					l.getId(), l.getDescricao()
			});
		}
	}

	private void fechar() {
		GerenciarLocaisVisao.this.dispose();
	...
	private void adicionarLocal() {
		AdicionarLocalVisao a;

		a = new AdicionarLocalVisao(controle, GerenciarLocaisVisao.this);
	...
	private void atualizarLocal() {
		AtualizaLocalVisao a;

		if(table.getSelectedRowCount() == 1) {
			a = new AtualizaLocalVisao(controle, GerenciarLocaisVisao.this, Integer.parseInt(table.getValueAt(table.getSelectedRow(), 0).toString()), table.getValueAt(table.getSelectedRow(), 1).toString());
	...
	private void deletarLocal() {
		...
		if(table.getSelectedRowCount() == 1) {
			d = new DeletarLocalVisao(controle, GerenciarLocaisVisao.this, Integer.parseInt(table.getValueAt(table.getSelectedRow(), 0).toString()), table.getValueAt(table.getSelectedRow(), 1).toString());
	...
	private void visualizarLocal() {
		VisualizarLocalVisao v;

		if(table.getSelectedRowCount() == 1) {
			v = new VisualizarLocalVisao(controle, GerenciarLocaisVisao.this, Integer.parseInt(table.getValueAt(table.getSelectedRow(), 0).toString()), table.getValueAt(table.getSelectedRow(), 1).toString());
