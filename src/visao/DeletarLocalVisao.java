...
public class DeletarLocalVisao extends JFrame{
	...
	public DeletarLocalVisao(LumanferControle controle, GerenciarLocaisVisao gerenciarLocaisVisao, int id, String desc) {
		super("Deletar Local");
	...
	private void fechar() {
		DeletarLocalVisao.this.dispose();
	...
	private void deletarLocal() {
		...
		if(!local.equals("")) {		
			if(id != 0)
				if(controle.removeLocal(id)) {
					JOptionPane.showMessageDialog(DeletarLocalVisao.this, String.format("Local deletado com sucesso"), "", JOptionPane.INFORMATION_MESSAGE);
					fechar();
				}else
					JOptionPane.showMessageDialog(DeletarLocalVisao.this, String.format("Erro ao deletar local"), "", JOptionPane.ERROR_MESSAGE);
		}
	}
